...
uint8_t cnt = 0;
char tf_data[4];
uint16_t distance = 0, distance_2 = 0, prev_dist = 0, prev_dist2 = 0;
float target_dist = 0;
...
int main() {
  ...
  Serial_int_setup();                //serial communication setup
  Serial.begin(9600);
  timer_setup(362);                     // timer setup
  while ((PINL & (1 << limit_pin))  | (PINL & (1 << limit_pin2))) {  //while either of the limit pin gets pressed
    PORTH &= ~((1 << dir) | (1 << dir2)); 
    if (PINL & (1 << limit_pin)) {   //for 1st stepper
      PORTB ^= (1 << pulse_pin);     //anticlockwise direction after limit pin is pressed
    }
    else {
      PORTB &= ~(1 << pulse_pin);    // clockwise direction till limit pin is not pressed
    }
    if (PINL & (1 << limit_pin2)) {  //for 2nd stepper
      PORTB ^= (1 << pulse_pin2);    //anticlockwise direction after limit pin is pressed
    }
    else {
      PORTB &= ~(1 << pulse_pin2);   // clockwise direction till limit pin is not pressed
    }
    _delay_us(500);                  //delay for pulse
  }
  prev_dist = distance;              //stores the instantaneous distance in prev dist till the time it starts
  value = distance;                  // stores the intantaneous distance in value after comparing every prev_dist
  prev_pos = 1;                    //setting initial pos to 100steps offset
  prev_pos2 = 0;                     //setting initial pos to 0steps offset
  _delay_ms(1000);
...
//    set_pos2(800);
    ...
    Serial.print("   ");
    Serial.print(prev_pos*0.1125  , 0);
    Serial.print(",");
    Serial.print(prev_dist);
     Serial.println(".");
    
    if (a == 0) {                     // for 1st half rotation
      if (prev_pos ==1)            // if it is at 0 degreee  100
      {
        set_pos(1300);                //scan till 180
        prev_dist = 10000;            //prev dist updated
      }
      else {                          //if it is not at 0
        angle_calculation(target_step, value);      
        set_pos2(target_step2);       //2nd tf mini stops at least dist
        if (prev_pos == 1300) {
          if((prev_dist2 > distance) && (( distance_2< (target_dist - 10)) || ( distance_2 > (target_dist + 10))) {        //   ***********
            prev_dist2 = distance
          }
          a = 1;
          prev_dist = 1200;
    ...
    else if (a == 1)                 //for 2nd half rotation
    {
      if (prev_pos == 1300)
      {
        set_pos(1);
  ...
  void targetStep()                     //to find the east value among all the scanned value
  {
    //  if (abs(prev_dist - target_dist) > 30){
    //    prev_dist = 10000;
    //  }
    if ((distance < prev_dist) && (distance < 10000) && (distance > 100))
    {
      prev_dist = distance;
      if (value > prev_dist){
      value = prev_dist;
