...
void loop() {
  ...
  Serial.print(phi);
  Serial.print("    ");
  Serial.print(distance);  //instantaneous distance by 1st tf mini
  Serial.print("    ");
  Serial.print(distance_2); //instantaneous distance by 2nd tf mini
  Serial.print("    ");
  Serial.print(target_dist);  //minimum distance detected by 2nd tf mini
  Serial.print("    ");
  Serial.print(value);  //minimum distance detected by 1st tf mini
  Serial.print("    ");
...
//  Serial.println(digitalRead(48)); 
  if (a == 0) {
    if (prev_pos == 0)
    {
      ...
      targetStep();
      if (prev_pos == 1300) {
        angle_calculation((target_step), value);//target_step - 4
        set_pos2((phi * 10.5)); //190   8.8889
        TIMSK3 |= (1 << OCIE3A);
        set_pos((target_step));  //-10              
        TIMSK1 |= (1 << OCIE1A);
  ...
  else if (a == 1)
  {
    if (prev_pos == 1300)
    {
      ...
      value = prev_dist;
      if (prev_pos == 0) {
        angle_calculation((target_step-35), value);
        set_pos2((phi * 8.8889) + 190);
        TIMSK3 |= (1 << OCIE3A);
        set_pos((target_step-35));                   
        TIMSK1 |= (1 << OCIE1A);
        a = 0;
      }
    }
...
void set_pos(int pos) {            //  set position for 2nd tf mini
  if (pos < prev_pos) {
    PORTH &= ~(1 << dir);
  }
  else {
...
void set_pos2(int pos) {             //  set position for 2nf tf mini
  if (pos < prev_pos2) {
    PORTH &= ~(1 << dir2);
  }
  else {
