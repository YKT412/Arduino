...
void base_motion_wrt_x (float deg_x, float base_speed) {
  //float rad = ((deg_x * M_PI) / 180);
  float rad = deg_x * 0.01744;
  float vx = cos(rad);
  float vy = sin(rad);
  n1 = (((-2 / 3.0) * vx) * base_speed);
  n2 = ((((1 / 3.0) * vx) + ((-1 / sqrt(3)) * vy)) * base_speed);
  n3 = ((((1 / 3.0) * vx) + ((1 / sqrt(3)) * vy)) * base_speed);
  motor_pwm (n1, n2, n3);
}
...
void loop () {
    ...
    if (ps3.cross){
      ...
      Serial.println("UP");
      if (ps3.left){
        base_motion_wrt_x (135, 3000);
      }
      else if (ps3.right){
         base_motion_wrt_x (45, 3000);
      }
      else {
        base_motion_wrt_x (90, 3000);
      }
//      _delay_ms(1000);
    }
    else if (ps3.down) {
      if (ps3.left){
        base_motion_wrt_x (225, 3000);
      }
      else if (ps3.right){
         base_motion_wrt_x (315, 3000);
      }
      else {
        base_motion_wrt_x (270, 3000);
      }
      Serial.println("DOWN");
//      motor_pwm (0, 0, 0);
//      _delay_ms(1000);
...
//      motor_pwm (0, 0, 0)
      base_motion_wrt_x (0, 3000);
//      _delay_ms(1000);
    }
    else if (ps3.left) {
      Serial.println("LEFT");
//      motor_pwm (0, 0, 0)
      base_motion_wrt_x (180, 3000);
//      _delay_ms(1000);
    }
    else {
      Serial.println("Not");
