...
double target_dist = 0, target_dist1 =0, target_dist2 =0, a , b, c;
int pulse1 = 0, prev_pos1 = 0, c = 0, a = 0, pulse2 = 0, prev_pos2 = 0, pos1 = 0, pos2 = 0;
unsigned int value1 = 0, steps = 3200, flag = 0 ;
double phi1 = 0, phi_rad1 = 0, theta1 = 0, theta_rad1 = 0 ,phi2 = 0,  phi_rad2 = 0, theta2 = 0, theta_rad2 = 0, C_rad = 0 , C;
...
void set_stepper() {
  if (!flag) {  
    ...
    if (set == 1000){
    dirf = (-1);
//    angle_calculation1 (target_step1, prev_dist1);
//    angle_calculation2 (target_step2, prev_dist2);
//    compare();
//   print_dist();
//    prev_dist2 = 65535;
//    prev_dist1 = 65535;
...
//    }
    ...
    print_dist();
    speed_calculation(target_dist1, target_dist2, phi1, phi2);
  }
}
...
void angle_calculation1 (float steps, float d) {
  ...
  target_dist1 = sq(d+3.6) + sq(20.25) - (d * 40.5 * cos(theta_rad1));
...
void angle_calculation2(float steps, float d) {
  ...
  target_dist2 = sq(d+4.0) + sq(20.25) - (d * 40.5 * cos(theta_rad2));
  target_dist2 = sqrt(target_dist2);
  phi_rad2 = asin(((sin(theta_rad2) * d) / target_dist2));
...
void speed_calculation(float d1, float d2, float A, float B );
{
  
  C = 180 - A - B;
  C_rad = C*0.0174;
  c= sq(d1) + sq(d2) - (d1 * d2 * cos(C_rad));
  c = sqrt(c);
  spd = c / t ;
