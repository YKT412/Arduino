...
float phi = 0, x = 34, phi_rad = 0, base = 0, vertical = 0, theta = 0, rad = 0;
...
void angle_calculation (int steps, unsigned int d) {
  theta = (steps * 0.1125);                    //theta= (target_step/1600)*180
  rad = (theta * 0.0174);                      //rad  =  (theta*pie/180)
  if (theta > 90) base = (-(d * cos (rad)));
  else base = (d * cos (rad));
  vertical = (d * sin(rad));
  if ((theta > 90) && (base > x)) {
    phi_rad = (3.14 - atan( vertical / -(base + x)));
  }
  else {
    phi_rad = atan ( vertical / (x + base));
  }
  phi = phi_rad * 57.3248;
}
