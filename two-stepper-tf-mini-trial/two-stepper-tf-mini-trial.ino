...
#define pulse_pin PB5   //stepper_1 pulse pin 11
#define limit_pin PE4   //stepper_1 limit pin 2
#define limit_pin2 PE5  //stepper_2 limit pin 3
#define dir PH6       //stepper_1 direction pin 9
#define pulse_pin2 PB4   //stepper_2 pulse pin 10
...
int main() {
  DDRG = 0x20;
  PORTG = 0x00;
  DDRE &= ~((1 << limit_pin) | (1 << limit_pin2));
  DDRB |= ((1 << pulse_pin) | (1 << pulse_pin2));
  DDRH |= ((1 << dir) | (1 << dir2));
  PORTE |= ((1 << limit_pin) | (1 << limit_pin2));
  Serial_int_setup();
  Serial.begin(115200);
  timer_setup(500);
  
  while ((PINE & (1 << limit_pin))) {     //when the limit switch is close the stepper pins will be high
    PORTH &= ~(1 << dir);
    PORTB ^= (1 << pulse_pin);
    _delay_us(500);
  }
  
   while ((PINE & (1 << limit_pin2))) {    //when the limit switch is close the stepper2 pins will be high
    PORTH &= ~(1 << dir2);
    PORTB ^= (1 << pulse_pin2);
    _delay_us(500);
  }
