#include "pinsetup_mega.h"
#define deg_to_steps 8.89

float  horizontal_angle = 0, vertical_angle = 0, velocity = 0, height = 85.0;
uint8_t c = 0, p = 0;
int prev_pos = 0,  pulse = 0, H_pos = 0;
uint16_t rpm = 0;
...
void rpm_calculation(float distance) {
  ...
  velocity = sqrt((2 * height * g) / (pow(sin(vertical_angle), 2)));892
//  velocity = 890;
//  vertical_angle = (asin((2*dist*980)/pow(velocity, 2)))/2;
