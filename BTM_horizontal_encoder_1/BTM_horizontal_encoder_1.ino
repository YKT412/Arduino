...
void  pid_set(unsigned int y)
{
  Serial.println("yes");



  if ( (EncoderOne + (6.67 * y)) < -5)
  {
    PORTH &= ~(1 << PINH3); //direction:  clockwise---> low
    //              if(abs(EncoderOne) > ((6.67*y)*0.6)){
    //              OCR4B = 200;
    //              }
    //              else{
    OCR4B = 200;
...
void set_degree(unsigned char x) {
  if ((x > x_p))
  {
    ...
    PORTH |= (1 << PINH3); //direction:   anitclockwise---> high
    if ((abs(EncoderOne)) > (6.67 * x)) // && (x = x_p))
    {
